...
import de.mineking.hexo.parse.ZeroOffsetLine
...
class BKENotationParser {
    ...
    fun `parse long sword`() {
        val board = "o A0 A2 x A1 A4 o B1.0 D4.0".parseBKENotation(zeroOffsetLine = ZeroOffsetLine.TopRight)
        assertEquals(
            mapOf(
                ...
                CellCoordinate(2, 0) to Cell(Player.O, turn = 3),
                CellCoordinate(-4, 0) to Cell(Player.O, turn = 3),
            ),
            board.cells,
        )
    }
    ...
    fun `parse x A0 A1 o A3 A4 x B0 B1`() {
        val board = "x A0 A1 o A3 A4 x B0 B1".parseBKENotation(zeroOffsetLine = ZeroOffsetLine.BottomLeft, chirality = Chirality.CounterClockwise)
        assertEquals(
            mapOf(
                CellCoordinate(0, 0) to Cell(Player.X, turn = 0),
                ...
                CellCoordinate(-2, 2) to Cell(Player.X, turn = 3),
                CellCoordinate(-1, 2) to Cell(Player.X, turn = 3),
            ),
            board.cells,
        )
    }
